...
for project in projects:
    for version in versions[project]:
        promise_data = csv.reader(open( "E:/data/promise_data/{}/{}.csv".format(project,version), 'r'))
        next(promise_data)
        all_count=0
        exist_count=0
        sequence_file=open('E:/sequence/{}_{}.txt'.format(project,version), 'w+', encoding='utf-8')
        ...
        for line in promise_data:            
            ...
            for pos_dir in dict_dir[project]:
                path =  "E:/Code/{}/{}/{}".format(project,version,pos_dir)
                file_path = path + "/" + file_name
                print(file_path)
                if os.path.exists(file_path):
                    file_sequence = to_seq(file_path)
                    sequence_file.write(str({"sequence": file_sequence, "bug": line[-1]})+"\n")
                else :
